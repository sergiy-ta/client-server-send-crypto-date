...
app.get('/crypt', (req, res) => {
    res.send(text_list.filter(object => object.ip === req.connection.remoteAddress).map(object => AesCtr.encrypt(object.text, "password", 128))); 
});

app.post('/crypt', jsonParser, (req, res) => {
    text_list.push({
        ip: req.connection.remoteAddress,
        text: AesCtr.decrypt(req.body.text, "password", 128)
    });
});
