class Aes {
    ...
    static cipher(input, w) {
        ...
        let state = [ [], [], [], [] ];  // initialise 4×Nb byte-array 'state' with input
        ...
        const output = new Array(4 * Nb);  // convert state to 1-d array before returning
...
class AesCtr extends Aes {
    ...
    static encrypt(plaintext, password, nBits) {
        ...
        // initialise 1st 8 bytes of counter block with nonce (NIST SP 800-38A): [0-1] = millisec,
...
document.getElementById("submit_button").addEventListener("click", function () {
    let body = {
        text: AesCtr.encrypt(input_text.value, "password", 128)
    };
...
function getTextList() {
    fetch('/crypt').then(
        ...
        response.json().then(function(text_list) {
            document.getElementById('text_list').innerHTML = "";
            console.log(text_list);
            text_list.forEach(text => {
                document.getElementById('text_list').innerHTML += "<li>" + AesCtr.decrypt(text, "password", 128) + "</li>";
            });
        });
    }).catch(function(err) {
...
function getCryptoTextList() {
    fetch('/crypt').then(
        ...
        // Examine the text in the response
        response.json().then(function(text_list) {
            document.getElementById('text_cryptos_list').innerHTML = "";
            text_list.forEach(text => {
                document.getElementById('text_cryptos_list').innerHTML += "<li>" + text + "</li>";
            });
        });
    }).catch(function(err) {
        console.log('Fetch Error :-S', err);
